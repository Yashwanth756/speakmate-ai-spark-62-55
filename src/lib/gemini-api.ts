...
// Bulk feedback for reflex challenge: each item = { prompt, response }
export const getBulkLanguageFeedback = async (
...
> => {
  ...
  for (let i = 0; i < items.length; i++) {
    const { prompt, response } = items[i];

    // We'll request a very thorough feedback for each answer.
    const promptText = `
      Analyze the following student's English answer to this question:
      ...
      {
        "corrected": "A corrected, natural-sounding version of the student's answer.",
        "explanation": "A clear, in-depth explanation of mistakes made in the answer.",
        "rulesBroken": ["A list of specific grammatical rules that were broken (as short descriptions)."],
        "grammarScore": number, // 0-100
        "vocabularyScore": number, // 0-100
        "fluencyScore": number, // 0-100
        "pronunciationScore": number, // 0-100 (best guess based on how the response sounds; mention 'N/A' if not possible)
        "precisionScore": number, // 0-100 (how precisely the answer matches the intent of the prompt)
        "speedScore": number // 0-100 (how concise/efficient the answer is)
      }
    ...
    try {
      ...
      try {
        ...
        // Try to extract a JSON object if model wrapped the response
        const match = text.match(/\{[\s\S]*?\}/);
        if (match) parsed = JSON.parse(match[0]);
        else throw new Error("Could not parse feedback JSON from Gemini.");
      }
      results.push({
        prompt,
        response,
      ...
      results.push({
        ...
        response,
        corrected: "",
        explanation: "Error analyzing answer: " + err.message,
        rulesBroken: [],
        grammarScore: 0,
        vocabularyScore: 0,
        fluencyScore: 0,
